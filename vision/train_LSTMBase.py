...
args['arch']="models/fast_channel/"
# args['pref']="data/new-data/icu_discharge/"
args['pref']="data/new-data/death/"
...
with open('data/xray_scanner.pickle', 'rb') as handle:
...
def read_fix_batch(path, id_list, k, random, inds=None):
    ...
    if len(id_list) > k:
        if not random:
            ...
            inds.sort()
        for ind in inds:
            if id_list[ind] in scan_dict:
                images_t_list.append(int(scan_dict[id_list[ind]]['MM_id'])+1)
            else:
                images_t_list.append(0)
    else:       
        for id in id_list:
            if id in scan_dict:
                images_t_list.append(int(scan_dict[id]['MM_id'])+1)
            else:
                images_t_list.append(0)
        for i in range(k - len(id_list)):
            images_t_list.append(0)
    t = torch.tensor(np.array(images_t_list))
    return t
